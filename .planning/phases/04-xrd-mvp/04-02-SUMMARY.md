...
plan: 02
subsystem: api
tags: [xrd, preprocessing, peak-detection, validation, backend]
requires:
  - phase: 04-01
    provides: XRD stable registration and normalized import contract for .xy/.dat/.cif
provides:
...
affects: [04-03-PLAN.md, xrd-processing, backend-batch, stable-validation]
tech-stack:
  added: []
  patterns:
    - Template-driven stable modality processing payloads with per-template defaults
    - Deterministic ranked peak extraction using prominence-first ordering with explicit tie-breakers
key-files:
  created: []
  modified:
